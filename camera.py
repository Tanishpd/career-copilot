...
def get_frame(imgData):
    ...
    # FIX: Use cv2.IMREAD_COLOR (1) instead of cv2.COLOR_BGR2GRAY (6) which is an invalid flag for imdecode
    ...
    for i in range(nums[0]):
        ...
        if int(classes[0][i]) == 67:
            ...
            mob_status = 1
        # Removed else block that was resetting mob_status to 0 incorrectly
    ...
    # Moved draw_outputs to end to avoid interfering with face detection
    ...
    if ret == True:
        ...
        for face in faces:
            if landmark_model is None:
                print("Error: Landmark model is None")
                continue
            ...
            with cv_lock:
                try:
                    ...
                    continue
            # print(f"DEBUG: Marks detected: {len(marks)}")
            ...
            # Lowered threshold from 48 to 30 for better sensitivity
            if ang1 >= 30:
                user_move1 = 2
                print('Head down')
            elif ang1 <= -30:
    ...
    # Draw YOLO boxes AFTER face detection
    ...
    try:
        with cv_lock:
            gaze.refresh(image)
            # frame = gaze.annotated_frame() # Not used in return dict
    ...
    print(f"Eye movements: {eye_movements}")
...
class VideoCamera(object):
    ...
    def get_frame(self):
        ...
        # Encode to jpg base64 for get_frame
        ret, jpeg = cv2.imencode('.jpg', image)
        jpg_as_text = base64.b64encode(jpeg)
        
        # Call the existing logic
        res = get_frame(jpg_as_text)
        
        # The result contains 'jpg_as_text' which is the ANNOTATED IMAGE
        # Decoding back to bytes for streaming
        annotated_jpg = base64.b64decode(res['jpg_as_text'])
        return annotated_jpg